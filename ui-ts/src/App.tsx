      ...
      <div className="main">
        <header className="topbar">
          ...
          <div className="topbar-right">
            ...
            <span className="pill">
              <span
                className={`dot ${v || state.scanning ? "live" : ""}`}
                style={{
                  background: v?.color ?? (state.scanning ? "var(--accent)" : "var(--text-3)"),
                  ["--dot-glow" as string]: v ? `color-mix(in srgb, ${v.color} 40%, transparent)` : undefined,
                }}
              />
              {v ? `${v.name} · ${v.modeLabel}` : state.scanning ? "Scanning" : "No device"}
            </span>
          </div>
        </header>
